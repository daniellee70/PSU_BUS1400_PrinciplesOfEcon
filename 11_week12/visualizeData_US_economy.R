...
# US indicators ----
US_indicators_tbl <- market_indicators_tbl %>%
    filter(indicator %in% c("Yield_Spread", "Home_Price")) %>%
    pivot_wider(names_from = indicator, values_from = value) %>%

    left_join(economic_indicators_tbl %>%
                  filter(indicator %in% c("Inflation", "Consumer_Sentiment")) %>%
                  pivot_wider(names_from = indicator, values_from = value)) %>%
...
indicator_desc <- as_labeller(
    c(`Home_Price` = "S&P/Case-Shiller 20-City Composite Home Price Index (year-over-year percent change)",
      `Consumer_Sentiment` = "Consumer Sentiment Index (year-over-year percent change)",
      `Inflation` = "Consumer Price Index (year-over-year percent change)",
      `Yield_Spread` = "Treasury Yield Spread between the 10 Year Note and the 3 Month Bill"))


US_indicators_fig <- US_indicators_tbl %>%
...
write_rds(US_indicators_fig, "00_data/fig/US_indicators_fig.rds")
